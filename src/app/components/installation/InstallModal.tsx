...
export default function InstallModal({ lessonProgress }: InstallModalProps) {
  ...
  const fireCelebration = () => {
    ...
    // Confetti settings optimized for a joyful celebration
    const defaults = {
      startVelocity: 45,
      spread: 70,
      ticks: 200,
      ...
      colors: ["#9b5c12", "#b99b6b", "#fdfaf1", "#FFD700", "#E5E4E2"],
    ...
    const interval: any = setInterval(() => {
      ...
      // Launch from the bottom-left corner
      confetti({
        ...defaults,
        particleCount,
        origin: { x: 0, y: 0.9 }, // Popping up from bottom left
        angle: 60, // Aiming toward the center
      });

      // Launch from the bottom-right corner
      confetti({
        ...defaults,
        particleCount,
        origin: { x: 1, y: 0.9 }, // Popping up from bottom right
        angle: 120, // Aiming toward the center
      });
    }, 250);
  };
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[101] flex items-end sm:items-center justify-center p-4">
        <div
          className={`relative bg-gradient-to-br ${content.bg} rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden border border-[#b99b6b]/40 animate-slide-up`}>
          ...
          <div className="p-8">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="relative overflow-hidden w-full py-4 rounded-2xl bg-[#9b5c12] text-white font-bold shadow-lg shadow-[#9b5c12]/30 active:scale-[0.98] hover:bg-[#864e0f] transition-all flex items-center justify-center gap-2">
                <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/40 to-transparent -translate-x-full animate-shimmer pointer-events-none" />
